...
CREATE POLICY "Users can view their own members" 
  ...
  USING (tree_id IN (SELECT id FROM public.family_trees WHERE user_id = (select auth.uid())));

CREATE POLICY "Users can create members in their tree" 
  ON public.family_members FOR INSERT 
  WITH CHECK (tree_id IN (SELECT id FROM public.family_trees WHERE user_id = (select auth.uid())));
